...
logger = get_file_logger(
    log_dir=log_dir,
    get_file_logger=os.path.join(log_dir, f"ranking_server_test__{formatted_time}.log"),
    also_print=True,
)
...
def test_rank(client):
    for fpath in glob.glob(
        "/N/u/baotruon/BigRed200/osome-csdl-ranking-challenge/data/extension_data/twitter_raw*.json"
    ):
