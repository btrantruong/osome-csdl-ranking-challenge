...
def test_count_top_named_entities(my_celery_app, celery_worker):
    # sends an asynchronous task to the Celery worker.
    # The delay method is a shortcut to the apply_async method, which queues the task for execution.
    # result_key: where to store the result in Redis.
    result_key = "my_worker_test:top_named_entities"
    result = count_top_named_entities.delay(10, "2017-05-31", "2017-06-01", result_key)
    success = result.get()
    assert success

    # get connection from redis, retrieve results
    ...
    # check that result was completed within 10 sec
    assert (
        dt.total_seconds() < 10
