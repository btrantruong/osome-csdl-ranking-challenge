    ...
    try:
        ...
        for i in range(len(urls_available_unshortened)):
            ...
            if domain in audience_diversity_domains:
                audience_diversity_val[urls_index[i]] = pd_audience_diversity_URLs.loc[pd_audience_diversity_URLs['private_domain'] == domain]['visitor_var'].values[0]
        ...
        for i in range(len(topics)):
            if int(topics[i]) != -1:
                audience_diversity_val[text_index_processed[i]] = topic_diversity[str(topics[i])]
