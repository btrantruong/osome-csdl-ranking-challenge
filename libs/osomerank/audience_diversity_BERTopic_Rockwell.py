...
logger = get_file_logger(os.path.join(libs_path, config.get("AUDIENCE_DIVERSITY", "log_dir")),
...
model_outpath = os.path.join(libs_path, config.get("AUDIENCE_DIVERSITY", "audience_diversity_rockwell"))
if not os.path.exists(model_outpath):
    os.makedirs(model_outpath)
topic_model.save(model_outpath, serialization="safetensors", save_ctfidf=True, save_embedding_model=embedding_model)
