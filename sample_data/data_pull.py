...
def data_puller(platform, x, seed_no, username):
    ...
    if platform.upper() == "FACEBOOK":
        df = pd.read_csv(
            os.path.join(
                script_dir, "facebook_data/processed/filtered_comment_post.csv"
            )
        )
        ...
        comments_grouped_by_post_id = df[df["type"] == "Comment"].groupby(
            "all_post_ids"
        )
    ...
    if platform.upper() == "REDDIT":
        df = pd.read_csv(
            os.path.join(script_dir, "reddit_data/processed/filtered_reddit_data.csv"),
            low_memory=False,
        )
        ...
        for _, post_row in sample.iterrows():  # change made here, was posts_df before
            ...
            post_item["engagements"] = {
                "upvotes": post_item.pop("upvotes", 0),
                "downvotes": post_item.pop("downvotes", 0),
            }
            ...
            related_comments = df[
                (df["type"] == "Comment") & (df["post_id"] == post_item["id"])
            ]
            for _, comment_row in related_comments.iterrows():
                comment_item = comment_row.to_dict()
                comment_item.pop("title", None)
                comment_item["engagements"] = {
                    "upvotes": comment_item.pop("upvotes", 0),
                    "downvotes": comment_item.pop("downvotes", 0),
                }
                final_items.append(comment_item)
    ...
    if platform.upper() == "TWITTER":
        df = pd.read_json(
            os.path.join(script_dir, "twitter_data/processed/filtered_jan_2023.json")
        )
        ...
        sample["simulated_replies"] = (
            round(
                (sample["followers_count"] * 0.001)
                + np.random.normal(loc=0, scale=noise_std, size=len(sample)),
                0,
            )
            .clip(lower=0)
            .astype(int)
        ...
        sample["simulated_reposts"] = (
            round(
                (sample["followers_count"] * 0.005)
                + np.random.normal(loc=0, scale=noise_std, size=len(sample)),
                0,
            )
            .clip(lower=0)
        ...
        sample["simulated_likes"] = (
            round(
                (sample["followers_count"] * 0.01)
                + np.random.normal(loc=0, scale=noise_std, size=len(sample)),
                0,
            )
            .clip(lower=0)
    ...
    json_dir = os.path.join(script_dir, f"{platform.lower()}_json")
